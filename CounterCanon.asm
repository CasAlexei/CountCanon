...
;**                                                                           **
;**               тип МК ATmega8 Тактовая частота 4.00 МГ      Ver.2          **
;**                                                                           **
;*******************************************************************************
;*******************************************************************************
/*  Fuse Bits:
...
RESET:
 ...
 ldi	temp,0b11001111                       ; инициализация порта B
 out	DDRB,temp

 ldi	temp,0b00000011                       ; инициализация порта C
 out	DDRC,temp

 ldi	temp,0b11111110                       ; инициализация порта D
 out	DDRD,temp
 
 ldi	temp,0b00110000                       ; запись сигналов на вывод порта B
 out	PORTB,temp      
...
;------------------------------------------------------------------------------
 ...
 ldi Temp,0b00000010
    ...
    ; 3 - 
    ; 2 - CS02 - 0 \
    ; 1 - CS01 - 1  _ управление тактовым сигналом (clk/8)
    ; 0 - CS00 - 0 /
 out TCCR0,Temp
...
MAIN_LOOP_CENA:
 ...
 sbis	PINC,4								; проверяем нажатия кнопки "OK" 
 rjmp	LOOP_RESET_COL						; если нажата, то переходим к программе CENA_COL
 ...
 sbis	PINC,2								; проверяем нажатия кнопки сброс 
;		pind,0
 rcall	RESET_ALL_CENA						; если нажата, то переходим к программе COUNT_M1
...
LOOP_RESET_CENA:
 ...
 clr	count								; Если кнопка OK нажата, то сбрасываем данные регистров
 clr	cenaL								;  цены и количества копий
 clr	cenaH
 rcall	DISPLAY_COL						; Отображаем количество копий на дисплее
 rcall	d500ms
...
COUNT_INC_ON:
 rcall	d20ms
 sbis	PIND,0								; Ждем снятия сигнала со входа
 rjmp	COUNT_INC_ON
